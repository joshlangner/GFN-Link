...
//
// C++ API for Grid SDK
// This defines the main C++ interface objects. See GRID_LinkSDK_cAPI.h for constants, typedefs 
// and enums.
//
// ===============================================================================================

#ifndef GRID_LINK_SDK_CPPAPI_H
#define GRID_LINK_SDK_CPPAPI_H
...
namespace GRIDLinkSDK
{
	...
	// Sets up communication and initializes both the IGRIDLink and IGRIDApplication objects
	GRIDLinkError InitializeGRIDLinkSDK();

	// GRID Link shutdown method. 
	// This method must be called as part of application shutdown or when GRID Link is no longer 
    // needed.
	// Performs GRID Link shutdown logic and cleans up resources.
	void ShutdownGRIDLinkSDK();

	// ============================================================================================
	// Interface object for communication with GRID
	// Application -> GRID communication
	// Application developers should call these methods at the appropriate locations in their code
	class IGRIDLink
	{
	public:
		/// <summary>
		/// Query to determine if running in a GRID environment or not
		/// </summary>
		/// <returns>true if running in GRID environment, false otherwise</returns>
		virtual bool IsGRIDEnabled() = 0;
		...
		virtual GRIDLinkError RequestKeyboardOverlayOpen(GRIDScreenPosition gspPosition = gspBottom) = 0;
		...
		/// </summary>
        /// <returns>gleSuccess on Success, error code otherwise</returns>
		virtual GRIDLinkError RequestKeyboardOverlayClose() = 0;

		/// <summary>
		/// Obtain a user specific GRID authentication token for backend user authentication.
		/// GRIDLink manages memory, it is not necessary to preallocate a buffer.
		/// </summary>
		/// <params>ppchToken (out) - pointer to token returned by this method</params>
		/// <returns>gleSuccess on Success, error code otherwise</returns>
        virtual GRIDLinkError RequestGRIDAccessToken(const char** ppchToken) = 0;

		/// <summary>
		/// Provides a storage location for GRID cloud save data (persistant user data)
		/// </summary>
        /// <params>ppchStoragePath (out) - Path to storage directory</params>
        /// <returns>gleSuccess on Success, error code otherwise</returns>
        virtual GRIDLinkError GetStorageLocation(const char** ppchStoragePath) = 0;

		/// <summary>
		/// Notified GRID that saving of data is complete and that data should be uploaded.
		/// </summary>
        /// <returns>gleSuccess on Success, error code otherwise</returns>
		virtual GRIDLinkError NotifyStorageChange() = 0;
	};

	// This returns an instantiated IGRIDLink object, from which the above methods can be called.
	IGRIDLink* Instance();

	// ============================================================================================
	// Interface object for communication with application
	// GRID -> Application communication
	// Application developers must fill in provided GRIDApplication class implementation.
    // This object is instantiated when InitializeGRIDLinkSDK() is called.
	class IGRIDApplication
	{
	public:
	...
	// This takes an IGRIDApplication implementation object, so allows an object defined in developer
	...
	// ShutdownGRIDLinkSDK() is called.
	GRIDLinkError InitializeGRIDLinkSDK(IGRIDApplication* applicationObj, bool bApplicationManagedMemory = false);
}

#endif GRID_LINK_SDK_CPPAPI_H
